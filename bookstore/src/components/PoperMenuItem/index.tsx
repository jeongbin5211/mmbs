...
function PoperMenuItem({ setAnchorEl }: Props) {
  ...
  const renderMenuGroup = (
    ...
    <>
      ...
      {/* 주어진 판별 함수 - isLastGroupItem을 사용해서 마지막 메뉴 아이템이 아닐 경우 구분자를 표시 */}
  ...
  return (
    ...
    >
      {/* 카테고리 리스트를 반복적으로 렌더링 */}
      {CATEGORY_LIST.map(category => renderMenuGroup(category, title => title === CATEGORY_LIST[CATEGORY_LIST.length - 1].title))}

      {/* 나이별 카테고리 아이템을 렌더링 */}
      { AGE_LIST.map(age => renderMenuGroup(age, title => title === '4~7세'))}
    </Box>
  );
}
